...
//            Pieces.Add(new Tuple<int, int>(0, 7), new Rook(new Tuple<int, int>(0, 7), Player.Black));
        ...
        private Dictionary<char, int> PositionCodes = new Dictionary<char, int>
        {
            ['A'] = 0,
            ['B'] = 1,
            ['C'] = 2,
        ...
        private List<char> PositionValues = new List<char>{'A', 'B', 'C', 'D', 'E', 'F', 'G'};

        public string TransformPosition(Tuple<int, int> pos)
        {
            return PositionValues[pos.Item2] + (8 - pos.Item1).ToString();
        }
        ...
        public Tuple<int, int> TransformPosition(string pos)
        {
            ...
            if (int.TryParse(pos[1].ToString(), out y))
                x = PositionCodes[pos[0]];
            else if (int.TryParse(pos[0].ToString(), out y))
                x = PositionCodes[pos[1]];
            else return null;
        ...
        public void PrintBoard()
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                for (int j = 0; j < 8; j++)
                {
                    if (Pieces.TryGetValue(new Tuple<int, int>(i, j), out ChessPiece piece))
                    {
